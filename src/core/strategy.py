...
def save_directional_signals(df, symbol, time_interval, signal_type,gan_model_name):
    """
    ...
    Args:
        ...
        :param gan_model_name:
    """
    # 定义信号保存的文件夹
    signals_dir = f"../../all_results/{gan_model_name}/{signal_type}_signals"
    ...
    if not filtered_signals_df.empty:
        # 构造文件名，将所有关键信息都包含在内
        signal_filename = f"{signal_type}_signals_{time_interval}_{symbol}.csv"
        signal_filepath = os.path.join(signals_dir, signal_filename)

        # 保存 CSV 文件，只包含 'date' 和 'signal' 列
        # filtered_signals_df[['date', 'signal','pred_class_up','pred_class_down','pred_class_hold']].to_csv(signal_filepath, index=False)
        filtered_signals_df.to_csv(signal_filepath, index=False)
        print(f"Signals for {symbol} saved to {signal_filepath}")
    else:
        print(f"Warning: No valid signals found for {symbol} at {time_interval} .")

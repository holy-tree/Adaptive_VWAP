...
interval_to_windows = {
    ...
    '30min': [15,30],
    '60min': [15, 30, 60],
    'D': [15, 30, 60, SEQ_LENGTH]  # 假设日线SEQ_LENGTH分钟
}
...
if __name__ == "__main__":
    for gan_model in gan_models:
        gan_model_path = f"../../models/trained/generators/{gan_model}"
        for model_name in ["our-transformer", "rnn", "lstm"]:
        # for model_name in ["our-transformer"]:
        ...
        #                    ]:
            for symbol in ["rb9999", "i9999", "ni9999", "OI9999", "AP9999"]:
                set_seed(42)
                # 参数设置
                ...
                parser.add_argument('--intervals', type=str, default='15min,30min,60min',
                                    help='逗号分隔的周期列表，如1min,5min,15min,30min,60min,D')
                parser.add_argument('--start_date', type=str, required=False, default="2025-04-01",
                ...
                filtered_time_interval_map = {}

                # 遍历原始映射
                for k, v in time_interval_map.items():
                    # 如果值（例如 '1min'）在 intervals 列表中
                    if v in intervals:
                        # 将该项添加到新字典中
                        filtered_time_interval_map[k] = v

                # 更新原始变量
                time_interval_map = filtered_time_interval_map
                ...
                gan_generators = load_trained_gan_generators(data_file.split('/')[-1].split('.csv')[0], device,gan_model_path,window_sizes)

                # 加载1分钟数据集
                dataset_1min = load_financial_data(data_file, model_input_seq_length, latent_dim)

                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                print(f"---**分析开始** | 设备: `{device}`")
                    ...
                    try:
                        ...
                        for trade_direction in ["buy", "sell"]:
                            ...
                            for execution_window in allowed_windows:
                                all_metrics_list = []
                                all_results_df_list = []
                                for _, trade_signal in filtered_trade_signal.iterrows():
                                    ...
                                    print(
                                        f"[{selected_interval_label}] 开始执行回测 | execution window starts"
                                    )
                                    print(
                                        f"[{selected_interval_label}] AR模型: {model_name} 资产: {symbol} 执行窗口: {execution_window}分钟"
                                    )
                                    print(
                                        f"[{selected_interval_label}] 选定信号时间: {signal_time}, 执行窗口: {execution_window}分钟, 信号索引: {signal_idx}"
                                    )
                                    print(
                                ...
                                if all_metrics_list:
                                    ...
                                    output_dir = os.path.join(
                                        ...
                                        model_name,
                                        symbol,
                                        f"{time_interval}_win{execution_window}"
                                    )
                                    os.makedirs(output_dir, exist_ok=True)

                                    # 命名并保存文件
                                    combined_results_df.to_csv(os.path.join(output_dir, "results_df.csv"), index=True)
